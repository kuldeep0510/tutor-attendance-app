...
import { User, Edit, Trash2, AlertTriangle, ChevronLeft, ChevronRight } from "lucide-react"
...
export function StudentListClient({ 
  ...
  const handleDelete = async () => {
    ...
    try {
      setStudents((currentStudents) => 
        currentStudents.filter((student) => student.id !== deletingStudent.id)
      )
  ...
  const handlePageChange = async (newPage: number) => {
    try {
      setIsLoading(true)
      const data = await getStudents(newPage, 10)
